...
class DockerGUI:
    ...
    def run_docker(self):
        ...
        # Docker run command
        command = f"/bin/bash -c 'source /emsdk/emsdk_env.sh && cd /usr/app && emcc {name}.cpp -I/usr/include/eigen3 -I/usr/app/include -s WASM=1 -s EXPORT_ES6=1 -s MODULARIZE=1 -o build/{name}.mjs'"
        ...
        subprocess.run(['docker', 'cp', f'{container_name}:/usr/app/build/{name}.mjs', destination])
        subprocess.run(['docker', 'cp', f'{container_name}:/usr/app/build/{name}.wasm', destination])
